...
export default function ToneDetailPage() {
  ...
  return (
    <div className="h-full min-h-0 flex-1 overflow-y-auto p-8 max-w-2xl">
      <button
        type="button"
        onClick={() => navigate('/')}
        className="mb-8 flex items-center gap-2 text-sm text-brand-subtext transition-colors hover:text-brand-text"
      >
        <ArrowLeft size={14} />
        Back to Library
      </button>
      ...
      <div className="mb-2 flex items-start justify-between gap-4">
        <h1 className="min-w-0 font-display text-4xl font-semibold leading-none tracking-tight text-brand-text lg:text-5xl">
          {tone.name}
        </h1>
        {!isGuest ? (
      ...
      <p className="mb-6 font-body text-xs text-brand-muted">Added {tone.createdAt}</p>
