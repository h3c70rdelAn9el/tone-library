...
export default function ToneCard({
  ...
  const favorited = useToneStore((s) => s.favoriteToneIds.includes(tone.id));
    ...
    <motion.div
      ...
      <div className="min-w-0  flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
          {tone.genreTags.slice(0, 5).map((tag) => (
            <TagBadge
              key={tag}
              tag={tag}
            />
          ))}
          {tone.genreTags.length > 5 ? (
            <span className="text-[10px] text-brand-muted">+{tone.genreTags.length - 5}</span>
          ) : null}
        </div>
      ...
      <div className="flex shrink-0 items-center gap-1">
        <button
          ...
          <Star
            size={16}
            className={favorited ? 'text-brand-accent fill-brand-accent' : 'text-brand-subtext'}
          />
        </button>
        <button
          type="button"
